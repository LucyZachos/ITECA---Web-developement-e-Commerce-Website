...
function displayCartItems() {
  ...
  if (cartItems && cartItems.length > 0) {
    cartItems.forEach(function (item) {
      ...
      // if quantity is 1, disable the minus button
      if (item.quantity === 1) {
    ...
    lineRow.innerHTML = '<td colspan="7" class="line"></td>';
    ...
    // hidden input fielld to staore the cart
...
//Fucntion to clear the cart when a user logs out
function clearCart() {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  cartIcons.forEach((cartIcon) => {
    cartIcon.addEventListener('click', () => {
      ...
      const alertSpan = cartIcon.nextElementSibling;
...
//Heart button event listener - Wishlist
document.addEventListener('DOMContentLoaded', function () {
  const heartIcons = document.querySelectorAll('.heart');
  heartIcons.forEach((heartIcon) => {
    heartIcon.addEventListener('click', () => {
      // Show added to heart alert
      const alertSpan = heartIcon.parentNode.querySelector('.alertHeart');
      alertSpan.style.display = 'block';
      // Set a alert timer
      setTimeout(() => {
        alertSpan.style.display = 'none';
